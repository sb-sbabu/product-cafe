...
import {
    Settings, X, Moon, Sun, Bell, BellOff, Clock,
    Zap, Coffee, BookOpen, MessageCircle, Volume2, VolumeX,
    Save, RotateCcw, Timer, Focus
} from 'lucide-react';
import { cn } from '../../lib/utils';
import { loadUserTaste, saveUserTaste, setQuietHours, clearQuietHours, type UserTaste } from '../../lib/daily-brew/barista-memory';
...
export const BrewSettings: React.FC<BrewSettingsProps> = ({ isOpen, onClose }) => {
    ...
    const handleSourcePreference = (source: BrewSource, delta: number) => {
        const current = taste.bySource[source] || 0;
        const newValue = Math.max(-100, Math.min(100, current + delta));
        setTaste(prev => ({
            ...prev,
            bySource: { ...prev.bySource, [source]: newValue }
        }));
    };
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
            ...
            <div className="relative w-full max-w-md mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-amber-50/50 to-orange-50/30">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-gradient-to-br from-amber-500 to-orange-500 rounded-xl">
                            <Settings className="w-4 h-4 text-white" />
                        </div>
                        <div>
                            <h2 className="font-bold text-gray-900">Brew Settings</h2>
                            <p className="text-xs text-gray-500">Customize your notification experience</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
