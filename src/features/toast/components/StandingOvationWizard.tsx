/**
 * StandingOvationWizard - For significant achievements
 * 5-step wizard: Recipients → Value → Story → Image → Award & Finalize
 */
...
import { COMPANY_VALUES, EXPERT_AREAS, TOAST_IMAGES, DEMO_USERS, AWARDS } from '../data';
...
export const StandingOvationWizard: React.FC<StandingOvationWizardProps> = ({
    isOpen,
    onClose,
}) => {
    // Store
    const { createRecognition, getCurrentUser, checkDailyLimits } = useToastStore();
    const currentUser = getCurrentUser();
    const dailyLimit = checkDailyLimits('STANDING_OVATION');
    ...
    const filteredUsers = useMemo(() => {
        ...
        let filtered = DEMO_USERS.filter(u =>
            u.id !== currentUser?.id && !selectedIds.includes(u.id)
        );
        if (searchQuery) {
        ...
        return filtered;
    }, [searchQuery, currentUser?.id, selectedRecipients]);

    // Recommended award based on selected value
    ...
    const recommendedImages = useMemo(() => {
        if (!selectedValue) return TOAST_IMAGES.slice(0, 8);
        return TOAST_IMAGES.filter(img => img.recommendedValues.includes(selectedValue)).slice(0, 8);
    }, [selectedValue]);

    // Add recipient
    const addRecipient = useCallback((user: typeof DEMO_USERS[0]) => {
    ...
    }, [selectedRecipients, selectedValue, selectedExpertAreas, message, impact, selectedImageId, selectedAward, notifyManagers, nominateForMonthly, currentUser, createRecognition, onClose]);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Modal */}
            <div className="relative w-full max-w-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl shadow-2xl border border-white/10 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                {/* Header */}
                    ...
                    {currentStep === 3 && (
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    placeholder="When the project was at risk, they stepped up and... Be specific about their actions and how they demonstrated the value you selected."
                                    value={message}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-white/80">What was the impact? (Optional but powerful)</label>
                                <textarea
                                    placeholder="Saved $X, improved NPS by Y points, prevented a critical issue..."
                                    value={impact}
                    ...
                    {currentStep === 4 && (
                        <div className="space-y-4">
                            <div>
                                ...
                                <p className="text-sm text-white/60">Pick an image that captures the spirit of this recognition</p>
                            </div>

                            {/* Recommended */}
                            <div className="space-y-2">
                                <p className="text-xs font-medium text-white/60 uppercase tracking-wider">
                                    Recommended for "{valueData?.shortName || 'this value'}"
                                </p>
                                <div className="grid grid-cols-4 gap-3">
                                    {recommendedImages.map(img => (
                                        <button
                                            ...
                                            className={`aspect-square rounded-xl overflow-hidden border-2 transition-all ${selectedImageId === img.id
                                                ? 'border-purple-500 ring-2 ring-purple-500/30 scale-105'
                                                : 'border-white/10 hover:border-white/30'
                                                }`}
                                        >
                                            <div className={`w-full h-full flex items-center justify-center text-2xl ${img.category === 'CELEBRATION' ? 'bg-gradient-to-br from-amber-500/30 to-orange-500/30' :
                            ...
                            {/* All images */}
                            <div className="space-y-2">
                                <p className="text-xs font-medium text-white/60 uppercase tracking-wider">All Images</p>
                                <div className="grid grid-cols-6 gap-2 max-h-40 overflow-y-auto p-1">
                                    {TOAST_IMAGES.map(img => (
                                        <button
                                            key={img.id}
                                            onClick={() => setSelectedImageId(img.id)}
                                            ...
                                            title={img.name}
                                        >
                                            <div className={`w-full h-full flex items-center justify-center text-lg ${img.category === 'CELEBRATION' ? 'bg-gradient-to-br from-amber-500/20 to-orange-500/20' :
                                                img.category === 'TEAMWORK' ? 'bg-gradient-to-br from-blue-500/20 to-cyan-500/20' :
                                                    img.category === 'INNOVATION' ? 'bg-gradient-to-br from-purple-500/20 to-pink-500/20' :
                                                        img.category === 'CARE' ? 'bg-gradient-to-br from-rose-500/20 to-red-500/20' :
                                                            img.category === 'INTEGRITY' ? 'bg-gradient-to-br from-indigo-500/20 to-blue-500/20' :
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="space-y-2 pt-4 border-t border-white/10">
                                ...
                                <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-3">
                                    ...
                                    {impact && (
                                        <p className="text-xs text-white/60 italic">Impact: {impact}</p>
                                    )}
                ...
                <div className="px-6 py-4 border-t border-white/10 bg-white/5 flex items-center justify-between">
                    ...
                    {currentStep < 5 ? (
                        ...
                        <button
                            ...
                            disabled={isSubmitting || !dailyLimit.allowed}
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold transition-all ${isSubmitting || !dailyLimit.allowed
                                ? 'bg-white/10 text-white/30 cursor-not-allowed'
                                : 'bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:from-purple-600 hover:to-pink-600 shadow-lg shadow-purple-500/25'
                                }`}
                        >
